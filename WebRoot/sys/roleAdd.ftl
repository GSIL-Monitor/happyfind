...
jQuery(document).ready(function(){
	...
	$.ajax({
                cache: true,
                type: "POST",
                url:"${request.contextPath}/roleAction!roleAddmenu.action",
...
<body>
 <form id="formid" action="roleAction!roleAddmenu.action" method="post">
	<div class="addbox userRight">
        ...
        <div class="addcon">
        	<ul>
            	<li>
                	<label>角色名称：</label>
                    <input name="role.rolename" type="text" value="" class="user defText"/>
                </li>
                <#list menulist as menu>
                <li class="mt10">
                	<span class="title">${menu.menudesc!}</span>
        			<input name="menuids" type="checkbox" value="${menu.menuid?string('#')}" id="rem" class="check"/><label for="rem">${menu.menuname!}</label>
                </li>
                 </#list>
            </ul>
        </div>
